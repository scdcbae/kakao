...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        Log.d("넘어온 위경도값",position);
        String[] str = position.split(",");
        double lat = Double.parseDouble(str[0]);
        double lng = Double.parseDouble(str[1]);

        // Add a marker in Sydney and move the camera
        LatLng now_position = new LatLng(lat, lng);
        mMap.addMarker(new MarkerOptions().position(now_position).title("Marker Now"));
        mMap.moveCamera(CameraUpdateFactory.newLatLngZoom(now_position, 16));
    }
}
